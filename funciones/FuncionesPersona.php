<?php
	class FuncionesPersona {
        ...
        public function obtenerFecha4($fecha, $ind_html = 1) {//Retorna la fecha - ejemplo: Miércoles 04 Dic/13
			$anio = date("y", $fecha);
			$mes = date("m", $fecha);
			$dia = date("d", $fecha);
			...
			$dia_sem_aux = $this->obtenerNombreDia($dia_sem, 1, $ind_html);
        ...
        public function obtenerFecha6($fecha) {//Retorna la fecha - ejemplo: 04/Dic/2013
			...
			return $dia."/".$mes_aux."/".$ano;
		}
		
		/**
		 * Formato (1: Largo - 2: Corto)
		 */
		private function obtenerNombreDia($dia_sem, $formato, $ind_html = 1) {
			$dia_sem_aux = "";
			switch ($dia_sem) {
				case "0":
					switch ($formato) {
				...
				case "3":
					switch ($formato) {
						case 1:
							$dia_sem_aux = ($ind_html == 1 ? 'Mi&eacute;rcoles' : "Miércoles");
							break;
						case 2:
							$dia_sem_aux = ($ind_html == 1 ? 'Mi&eacute;' : "Mié");
							break;
					}
					break;
				case "4":
				...
				case "6":
					switch ($formato) {
						case 1:
							$dia_sem_aux = ($ind_html == 1 ? 'S&aacute;bado' : "Sábado");
							break;
						case 2:
							$dia_sem_aux = ($ind_html == 1 ? 'S&aacute;b' : "Sáb");
							break;
					}
					break;
				default:
		 ...
		 * Formato (1: Largo - 2: Corto)
		 */
		public function obtenerNombreMes($mes, $formato) {
			$mes_aux = "";
			switch ($mes) {
				case '01':
					switch ($formato) {
