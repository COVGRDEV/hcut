...
class Combo_Box {
    ...
    public function getComboGrupoDb($nombre, $activo, $tabla_grupo, $campos_grupo, $tabla, $campos, $mensaje, $al_cambio = 0, $activo_combo = '0', $estilo = '', $index = '', $class = '') {
        ...
        $lista_campos = explode(",", $campos);
        $lista_campos_grupo = explode(",", $campos_grupo);

        foreach ($tabla_grupo as $fila_grupo) {
            $id_grupo = $fila_grupo[trim($lista_campos_grupo[0])];
            $nombre_grupo = $fila_grupo[trim($lista_campos_grupo[1])];
            ...
            while (@$tabla[$i][trim($lista_campos[1])] || @$tabla[$i][trim($lista_campos[1])] == "0") {

                $id = $tabla[$i][trim($lista_campos[1])];
                $id_padre = $tabla[$i][trim($lista_campos[0])];

                if ($id_padre == $id_grupo) {

                    $nombre_valor = $tabla[$i][trim($lista_campos[2])];
