<?php
	class JSON_WebService {
		private $methods, $args, $strcall, $keyVal;
		public function __construct($key, $rawData) {
			$this->keyVal = $key;
		...
		private function call($name, $args) {
			if ($this->methods[$name] == true) {
				...
				if (isset($args["APIKey"])) {
					if (sha1($args["APIKey"]) == $this->keyVal) {
						$result = call_user_func_array($name, $args);
						return json_encode(json_encode($result));
					} else {
						return json_encode(json_encode(array("error_message" => "Acceso no autorizado")));
					}
				} else {
					return json_encode(json_encode(array("error_message" => "Error de identificación")));
				}
			}
		}
