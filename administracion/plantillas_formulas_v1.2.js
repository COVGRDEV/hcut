...
function imprimir() {
    ...
    var text_despacho = $("#text_formula").val();
...
function guardarPlantilla(accion) {
    var result = 0;
	var text_formula = $("#text_formula").val();
    var idCodigo = $('#hddidFormula').val();
	
    $("#text_formula").removeClass("borde_error");
    if (text_formula == "") {
        $("#text_formula").addClass("borde_error");
        result = 1;
    }
    ...
    if (result == 0) {//Guarda
        ...
        var text_formula2 = str_encode($("#text_formula").val());
