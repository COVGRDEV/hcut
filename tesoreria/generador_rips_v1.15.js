	...
	if (ind_us == 1) {
		...
		for (var i = 0; i < cant_registros_us; i++) {
			params += "&id_rips_us_" + i + "=" + $("#hdd_id_rips_us_" + i).val() +
					  ...
					  "&fecha_nacimiento_us_" + i + "=" + $("#txt_fecha_nacimiento_us_" + i).val() +
					  "&cod_pais_us_" + i + "=" + $("#txt_cod_pais_us_" + i).val() +
...
function generar_rips() {
	if (validar_generar_rips()) {
		...
		//var resultado = $("#hdd_result_act_directo").val(1);
...
function confirmar_generar_rips_directo() {
	...
	$("#d_interno").html(
		'<table class="datagrid" border="0" cellpadding="5" cellspacing="0" align="center" style="width:100%">' +
			...
			'<tr>' +
				'<th align="center" style="width:5%;border: 1px solid #fff;">' +
					'<input type="button" id="btn_cancelar_si" nombre="btn_cancelar_si" value="Aceptar" class="btnPrincipal" onclick="validar_tipo_factura_rips_directo();"/>' +
					'&nbsp;&nbsp;' +
					'<input type="button" id="btn_cancelar_no" nombre="btn_cancelar_no" value="Cancelar" class="btnPrincipal" onclick="cerrar_div_centro();"/>' +
				'</th>' +
			'</tr>' +
...
function validar_tipo_factura_rips_directo(){
	$("#btn_generar_rips").attr("disabled", "disabled");
	cerrar_div_centro();
	
   	var tipo_factura =  $("#cmb_tipo_factura").val();	

	if(tipo_factura==1){
		$("#fondo_negro").css("display", "block");
		$("#d_centro").slideDown(500).css("display", "block");
		
		$("#d_interno").html(
			'<h4 style = "margin-bottom: 40px; margin-top: 30px; ">Ingrese el n&uacutemero de factura &uacutenico, aseg&uacuterese que el n&uacutemero sea correcto.<br /></h4>' +
			'<table class="datagrid" border="0" cellpadding="5" cellspacing="0" align="center" style=" margin-bottom: 40px; width:60%">' +
			
				'<tr align="center" style="  width:50%">' +
					'<td align="right" class="msg_alerta" style="width:20%; border: 1px solid #fff;">' +
						'<label class="inline">N&uacute;mero de factura:&nbsp;</label>'+
					'</td>' +
					'<td align="center" class="msg_alerta" style="width:40%; border: 1px solid #fff;">' +
						'<input type="text" id="num_factura_unico" name="num_factura_unico" id="num_factura_unico" class="" onblur="trim_cadena(this);" maxlength="20" />' +
					'</td>' +
					'<td align="center" style="width:30%; border: 1px solid #fff;">' +
						'<input type="button" style="width:80%;  id="btn_cancelar_si" name="btn_cancelar_si" value="Aceptar" class="btnPrincipal peq" onclick="validar_factura_no_vacia();"/>' +
					'</td>' +
				'</tr>' +
			'</table>');
		posicionarDivFlotante("d_centro");	
		...
		$("#d_interno").html('<input type="hidden" id="num_factura_unico" name="num_factura_unico" value="-1"/>');
		generar_rips_directo();
	}	
	
}

function validar_factura_no_vacia(){
	
		var numero_factura =  $("#num_factura_unico").val();
		if(numero_factura !== ''){
			generar_rips_directo();	
			cerrar_div_centro();	
			$("#d_generando_rips").show();	
		} else{
			window.alert("¡Error!, El campo número de factura no puede estar vacío");
		}
	}
...
function continuar_generar_rips(tipo) {
	...
	switch (tipo) {
		...
		params += "&id_rips=0&id_convenio=" + $("#cmb_convenio").val() +
					  ...
					  "&id_prestador=" + $("#cmb_prestador").val() +
					  "&ind_sin_np=" + ($("#chk_sin_np").is(":checked") ? 1 : 0) +
					  "&num_factura=" + $("#num_factura_unico").val();
			break;
...
function terminar_generar_rips() {
	...
	$("#btn_generar_rips").removeAttr("disabled");
	//var resultado = $("#hdd_result_act_directo").val();
	//if (resultado > 0) {
		alert("RIPS generados con \xe9xito.");
		cargar_rips_disponibles();
	//} else {
	//	alert("Error interno al generar los archivos del RIPS (" + resultado + ").");
	//}
}
