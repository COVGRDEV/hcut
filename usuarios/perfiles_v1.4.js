...
function crear_perfil() {
	...
	var cmb_recibe_citas = $("#cmb_recibe_citas").val();
	var cmb_atiende_cirugias = $("#cmb_atiende_cirugias").val();
	var cmb_menu_inicio = $("#cmb_menu_inicio").val();
	var cmb_registrar_pagos = $("#cmb_registrar_pagos").val();
	var cmb_modificar_pagos = $("#cmb_modificar_pagos").val();
	var hdd_idmenus = $("#hdd_idmenus").val();
	var array_menus = hdd_idmenus.split("-");
	...
	var params = "opcion=3&txt_nombre_perfil=" + txt_nombre_perfil +
				 ...
				 "&cmb_recibe_citas=" + cmb_recibe_citas +
				 "&cmb_atiende_cirugias=" + cmb_atiende_cirugias +
				 "&cmb_menu_inicio=" + cmb_menu_inicio +
				 "&cmb_registrar_pagos=" + cmb_registrar_pagos +
				 "&cmb_modificar_pagos=" + cmb_modificar_pagos +
	...
	for (var k in array_menus) {
		...
		if (menu != "") {
			var cmb_permiso = $("#cmb_permiso_" + menu).val();
			params += "&cmb_permiso_" + menu + "=" + cmb_permiso;
		}
	}
...
function validar_editar_perfil() {
    $("#frm_perfiles").validate({
        ...
        submitHandler: function() {
            var hdd_idmenus = $("#hdd_idmenus").val();
            var array_menus = hdd_idmenus.split("-");
            var bandera = false; //Para saber si hay un menu seleccionado
            for (var k in array_menus) {
                var menu = array_menus[k];
                if (menu != "") {
                    var cmb_permiso = $("#cmb_permiso_" + menu).val();
...
function editar_perfil() {
	...
	var cmb_registrar_pagos = $("#cmb_registrar_pagos").val();
	var cmb_modificar_pagos = $("#cmb_modificar_pagos").val();
	var cmb_modificar_autorizaciones = $("#cmb_modificar_autorizaciones").val();
    var cmb_activo = $("#cmb_activo").val();
    var hdd_idmenus = $("#hdd_idmenus").val();
    var array_menus = hdd_idmenus.split("-");

    var params = "opcion=4&txt_nombre_perfil=" + str_encode(txt_nombre_perfil) +
				 "&txt_desc_perfil=" + str_encode(txt_desc_perfil) +
				 "&cmb_atiende_consulta=" + cmb_atiende_consulta +
				 "&cmb_recibe_citas=" + cmb_recibe_citas +
				 "&cmb_atiende_cirugias=" + cmb_atiende_cirugias +
				 "&cmb_menu_inicio=" + cmb_menu_inicio +
				 ...
				 "&cmb_modificar_autorizaciones=" + cmb_modificar_autorizaciones +
				 "&hdd_idmenus=" + hdd_idmenus;
...
function seleccionar_atiende_consulta(ind_atiende) {
	if (ind_atiende == "1") {
		$("#cmb_recibe_citas").attr("disabled", false);
	} else {
		$("#cmb_recibe_citas").val("0");
		$("#cmb_recibe_citas").attr("disabled", true);
	}
}
