function cargar_formula(texto_formula) {
	...
	var texto_descripcion_formula = text_despacho + '\n' + str_decode(texto_formula);
	
	//Se cargar a la caja de texto
	$("#text_despacho_" + indice).val(texto_descripcion_formula);
}
	...
	if (result == 0) {
		...
		for (var i = 0; i < cant_formulas; i++) {
			...
			if (bol_existe_aux) {
				...
				params += "&num_carnet_" + cont_aux + "=" + str_encode($("#txt_num_carnet_" + i).val());
				params += "&formula_medica_" + cont_aux + "=" + str_encode(text_despacho);
