...
function guardar_despacho(id_tipo) {
	...
	if (result == 0) {
		...
		var params = "opcion=1&text_despacho=" + str_encode(text_despacho) +
					 ...
					 "&text_remitido=" + str_encode(text_remitido) +
					 "&id_tipo=" + id_tipo;
...
function mostrar_formula(id){
	var estado_div=$("#detalle_formula_"+id).css("display");
	
	if(estado_div == 'none'){
		$("#detalle_formula_"+id).css("display", "");
	}
	else{
		$("#detalle_formula_"+id).css("display", "none");
	}
}
