function buscar_cotizaciones() {
	if (validar_buscar_cotizaciones()) {
		var params = "opcion=1&txt_paciente=" + str_encode($("#txt_paciente_b").val()) +
					 ...
					 "&id_usuario_prof=" + $("#cmb_usuario_prof_b").val() +
					 "&id_lugar_cita=" + $("#cmb_lugar_cita_b").val();
...
function validar_buscar_cotizaciones() {
	...
	if (($("#txt_fecha_ini_b").val() == "" || $("#txt_fecha_fin_b").val() == "")) {
		...
		$("#contenedor_error").html("Losc ampos marcados en rojo son obligatorios");
		result = false;
	}
