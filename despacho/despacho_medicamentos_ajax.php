...
switch ($opcion) {
				...
				if($tipo_acceso_menu == 2 && $formulacion["estado_formula_medicamento"] <> 1){ ?>
					<div style="margin-top:50px; ">
						<table style="width: 550px; margin: 0 auto;">
								...
								<tr>
									<td align="left">
										<input type="text" id="txtNumDespacho" name="txtNumDespacho" style="color:#399000s">
									</td>
								</tr>
						</table>
					...
					<div style="margin-bottom:50px;">
						<input class="btnPrincipal" type="button" value="Anular descarga" id="btnAnular" name="btnAnular" onclick="eliminar_medicamento();">
    ...
    case "3"://Realiza el despacho
        ...
        if ($banderaPasoUno) {//Sí el paso 1 del Stored Procedure es exitoso            
	...
	case "6": //Realiza la anulación 
		...
		$codDespacho 	     			=  	$utilidades->str_decode($_POST["codDespacho"]);
		
		
		//Obtiene datos para SIESA
		$objLugarDespacho = $dbFormulasMedicamentos->getLugarUltimoDespachosByFormulacion($id_formula);
        $sedeDetalle = $dbListas->getSedesDetalle($objLugarDespacho["id_lugar_desp"]);
		...
		$medicamentos_despachados = $dbFormulasMedicamentos->getMedicamentosDespachadosDet($codDespacho);
		//var_dump($medicamentos_despachados);
		...
		foreach($medicamentos_despachados as $value){
			$cod_siesa = $value['cod_item_siesa'];
			$lote = $value['lote_medicamento'];
			$cantidad = $value["cantidad_medicamento_desp_det"];
			...
			$costo = $costo == "" ? $value['costo_unitario'] : $costo;
		...
		if($resultado == 1){
				...
				$rta = $dbFormulasMedicamentos->cambiarEstadoFormulaMedicamentos($medicamentos_despachados, $codDespacho, $id_usuario);
				if($rta<0){
					//Error en la anulación por base de datos
					$resultado="Ocurrió un error al anular en base de datos.";
					$rta =-2;	
				}				
		}else{
		...
		?>
			...
			<input type="hidden" id="hdd_result_siesa_anulacion_medicamento" name="hdd_result_siesa_anulacion_medicamento" value="<?php echo($resultado); ?>" />
