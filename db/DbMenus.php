...
class DbMenus extends DbConexion {
    ...
    public function getListaMenus2($idUsuario) {
        try {
            $sql = "SELECT DISTINCT m.id_menu, IFNULL(MAX(p.tipo_acceso), 0) AS tipo_acceso, m.nombre_menu, m.pagina_menu, m.id_menu_padre 
					FROM menus m
    ...
    public function getMenuTipoCitaEstado($id_tipo_cita, $id_estado_atencion) {
        try {
            $sql = "SELECT M.*
					...
					OR CD.id_estado_atencion+1=".$id_estado_atencion.")
					ORDER BY CD.orden";
    ...
    public function getMenuInicioUsuario($id_usuario) {
        try {
            $sql = "SELECT * FROM (
						...
						INNER JOIN permisos PR ON P.id_perfil=PR.id_perfil AND P.id_menu_inicio=PR.id_menu
    ...
    public function getMenuTipoReg($id_tipo_reg) {
        try {
            $sql = "SELECT M.*
