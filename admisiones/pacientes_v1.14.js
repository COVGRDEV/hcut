        ...
        submitHandler: function () {
            ...
            if (bandera_continuar) {
                if (email_aux) {
                    ...
                    alert_basico("", 'Debe escribir un formato de correo válido ej: email@email.com', "error");
                }
            }
        },
    });
...
function guardar_paciente(tipo) {
	...
	var ind_habeas_data = "";
	if($('#ind_habeas_1').attr('checked')){
		ind_habeas_data = $('#ind_habeas_1').val();	
	}else if(!$('#ind_habeas_1').attr('checked') && !$('#ind_habeas_0').attr('checked')){
...
function validarImportarPacientes() {
    ...
    if (ind_importar == 1) {
        ...
        swal({//Ventana temporal que muestra el progreso del ajax...
            ...
            onOpen: () => {
                llamarAjaxUploadFiles("pacientes_ajax.php", params, "d_guardar_paciente", "validarImportacionPacientes()", "d_barra_progreso_adj", "fileISS");
            }
        });
