...
function formularioFlotanteFocus() {//Función que asegura el puntero del mouse en el input del formulario flotante
...
function mostrarFormularioMercadeo(){
	
	var id_paciente = $("#hdd_id_paciente").val();
	var categoria = $("#hdd_categoria_mercadeo").val();
	var subcategoria = $("#hdd_subcategoria_mercadeo").val();	 
	var otro="" //$.trim($("#txt_otro_merc").val());
	var remitido=""// $.trim($("#txt_remitido_merc").val());
	var referido= $.trim($("#txt_referido_merc").val()) == "" ? $("#hdd_referido_mercadeo").val() : $.trim($("#txt_referido_merc").val());
	//var referido= $.trim($("#txt_referido_merc").val());
	var params = "opcion=21&flotante=" + 1  + 
	...
	var funcion = "";
	if(categoria =="95"){
		funcion="seleccionar_mercadeo();";
	}
	llamarAjax("admision_ajax.php", params, "d_interno", funcion) ;
...
function guardar_datos_mercadeo(){
	
	var id_paciente = $.trim($("#hdd_id_paciente_aux").val());
	var categoria = $.trim($("#cmb_lista_merc").val().trim());
	var subcategoria = $.trim($("#cmb_subcategoria_merc").val());
	var otro = $.trim($("#txt_otro_merc").val());
	var remitido = $.trim($("#txt_remitido_merc").val());
	var referido =  $.trim($("#txt_referido_merc").val()); 
	var ind_continuar_mercadeo = false;
	
	if(categoria != '' && (subcategoria != '' || otro != '' || remitido != '' || referido !='')){
		ind_continuar_mercadeo = true;	
	    var params = "opcion=22&id_paciente=" + id_paciente  + 
								"&categoria=" + categoria +
								"&subcategoria=" + subcategoria +
								"&otro=" + otro +
								"&referido=" + referido +
   	 	...
   	 	llamarAjax("admision_ajax.php", params, "d_guardar_datos_mercadeo", "continuar_guardar_datos_mercadeo();");
	
	}else{
		
		swal({
			title: "Formulario mercadeo",
			type: "error",
			text: "Por lo menos, un campo del formulario de mercadeo debe ser llenado",
...
function seleccionar_mercadeo(){
	var id_lista_mercadeo = $("#cmb_lista_merc").val();
	var remitido = "";
	var otro = "";//$("#hdd_otro_mercadeo").val();
	var referido=$("#hdd_referido_mercadeo").val();
	var params = "opcion=23&id_lista_mercadeo=" + id_lista_mercadeo +
	 						"&remitido=" + remitido +
							"&referido=" + referido + 	
							"&otro=" + otro;
							
  	llamarAjax("admision_ajax.php", params, "d_subcategoria", "");

	
}


function continuar_guardar_datos_mercadeo(){
	
	var id_paciente = $("#hdd_id_paciente").val();
	
	$("#hdd_categoria_mercadeo").val($("#hdd_categoria_" + id_paciente).val());
	$("#hdd_subcategoria_mercadeo").val($("#hdd_subcategoria_" + id_paciente).val());
	$("#hdd_remitido_mercadeo").val($("#hdd_remitido_" + id_paciente).val());
	$("#hdd_otro_mercadeo").val($("#hdd_otro_" + id_paciente).val());
	$("#hdd_referido_mercadeo").val($("#hdd_referido_" + id_paciente).val());
...
function seleccionUsuario(id) {//Evento clic en el formulario flotante: Buscar Usuario
    ...
    if (confirm("\u00BFConfirma que desea seleccionar el paciente " + nombre_completo_aux + "?")) {
	    ...
	    $("#hdd_foto_paciente").val($("#hdd_foto_paciente_" + id).val());
...
function guardar_admision() {
    $("#frmAdmision").validate({
        ...
        submitHandler: function (form) {
			...
			var txt_num_poliza = "";
			var txt_num_mipress = "";
			var txt_num_ent_mipress = "";
			
			
            if ($("#cmb_departamento").is(":visible")) {
                cmb_departamento = $("#cmb_departamento").val().length > 0 ? $("#cmb_departamento").val() : "-1";//-1 el objeto está visible
            } else {
                cmb_departamento = "";//-2 El objeto no está visible
            }
			
            if ($("#cmb_municipio").is(":visible")) {
                cmb_municipio = $("#cmb_municipio").val().length > 0 ? $("#cmb_municipio").val() : "-1";//-1 el objeto está visible
            } else {
                cmb_municipio = "";//-2 El objeto no está visible
            }
		
			
			if ($("#txt_nombre_dep").is(":visible")) {
				txt_nombre_dep = $("#txt_nombre_dep").val().length > 0 ? $("#txt_nombre_dep").val() : "-1";//-1 el objeto está visible
			} else {
				txt_nombre_dep = "";//-2 El objeto no está visible
			}
			
			if ($("#txt_nombre_mun").is(":visible")) {
				txt_nombre_mun = $("#txt_nombre_mun").val().length > 0 ? $("#txt_nombre_mun").val() : "-1";//-1 el objeto está visible
			} else {
				txt_nombre_mun = "";//-2 El objeto no está visible
			}
			...
			if ($("#td_num_poliza").is(":visible")) {
				txt_num_poliza = $("#txt_num_poliza").val().length > 0 ? $("#txt_num_poliza").val() : "-1";
			} else {
				txt_num_poliza = "";
			}
			
			if ($("#td_num_mipress").is(":visible")) {
				txt_num_mipress = $("#txt_num_mipress").val().length > 0 ? $("#txt_num_mipress").val() : "-1";
			} else {
				txt_num_mipress = "";
			}
			
			if ($("#td_num_mipress").is(":visible")) {
				txt_num_ent_mipress = $("#txt_num_ent_mipress").val().length > 0 ? $("#txt_num_ent_mipress").val() : "-1";
			} else {
				txt_num_ent_mipress = "";
			}
            ...
            if (cmb_departamento === "-1" || cmb_municipio === "-1" || txt_nombre_dep === "-1" || txt_nombre_mun === "-1"
					|| cmd_profesionalAtiende === "-1" || txt_nombre_cirugia === "-1"
                    ...
                    if (ind_continuar) {
                        ...
                        for (i = 1; i <= filas; i++) {
                            if (isObject(document.getElementById("tr_precios_" + i))) {
                                ...
                                if (cantidad_aux <= 0) {
                                    $("#contenedor_error").css("display", "block");
                                    $("#contenedor_error").html("Todas las cantidades de los productos deben ser mayores a cero (0).");
                                    ind_continuar = false;
                                    break;
                                }
                            }
                            ...
                            for (i = 1; i <= filas; i++) {
                                if (isObject(document.getElementById("tr_precios_" + i))) {
                                    if (trim($("#txt_valor_pago_" + i).val()) == "" || trim($("#txt_valor_cuota_" + i).val()) == "") {
                                        $("#contenedor_error").css("display", "block");
                                        $("#contenedor_error").html("Los valores unitarios y de cuota moderadora de los productos no deben ser vacíos.");
                                        ind_continuar = false;
                                        break;
                                    }
                    ...
                    if ($("#hdd_numero_documento_ori").val() != "" && $("#txt_id").val() != $("#hdd_numero_documento_ori").val()) {
                        ...
                        var codigohtml =
							'<div class="encabezado">' +
								'<h3>Cambio de identificación del paciente</h3>' +
							'</div>' +
							'<br />' +
							'<table style="width:100%; font-size:10pt;">' +
								...
								'<tr>' +
									'<td align="center" colspan="4">' +
										'<h5>' +
											'Los datos de identificación han cambiado, ¿confirma que se trata del mismo paciente?' +
										'</h5>' +
									'</td>' +
								'</tr>' +
								'<tr>' +
...
function continuar_guardar_admision_2() {
	var id_lugar_session = $.trim($("#hdd_id_lugar_session").val());
	...
	//Datos del formulario de mercadeo
	 ...
	 if(id_lugar_session === "491" || id_lugar_session === "466" || id_lugar_session === "511" || id_lugar_session === "465" || id_lugar_session === "464"){
			var categoria = $("#hdd_categoria_" + id_paciente).val();
			var subcategoria = $("#hdd_subcategoria_" + id_paciente).val();
			var remitido = $("#hdd_remitido_" + id_paciente).val();
			var referido = $("#hdd_referido_" + id_paciente).val();
	 }else{
		categoria = 0; subcategoria = 0; remitido = 0; otro = 0; referido=0;
			
	 }
	 
	$("#hdd_categoria_mercadeo").val(categoria);
	$("#hdd_subcategoria_mercadeo").val(subcategoria);
	$("#hdd_remitido_mercadeo").val(remitido);
	$("#hdd_referido_mercadeo").val(referido);
	 
	if(id_lugar_session !="464"){
		if($("#hdd_categoria_mercadeo").val() != '' && ($("#hdd_subcategoria_mercadeo").val() != '' ||
			$("#hdd_remitido_mercadeo").val() != '' || $("#hdd_referido_mercadeo").val())){
			 ind_continuar_mercadeo = true;						
		}
	}else{
		ind_continuar_mercadeo = true;		
    ...
    if (tmp_pagos_detalle <= 0 || ind_continuar_mercadeo == false) {
		if(ind_continuar_mercadeo == false){
		$("#contenedor_error").css("display", "block");
		$("#contenedor_error").html("Los campos de mercadeo no pueden estar vacíos");
		$("#btnNuevo").removeAttr("disabled");
		window.scrollTo(0, 0);
		}else{
        $("#contenedor_error").css("display", "block");
		...
		if ($("#td_num_poliza").is(":visible")) {
            num_poliza = $("#txt_num_poliza").val().length > 0 ? $("#txt_num_poliza").val() : "-1";//-1 el objeto está visible
        } else {
            num_poliza = "";//-2 El objeto no está visible
        }
		if ($("#td_num_poliza").is(":visible")) {
            num_mipress = $("#txt_num_mipress").val().length > 0 ? $("#txt_num_mipress").val() : "-1";//-1 el objeto está visible
        } else {
            num_mipress = "";//-2 El objeto no está visible
        }
		if ($("#td_num_ent_mipress").is(":visible")) {
            num_ent_mipress = $("#txt_num_ent_mipress").val().length > 0 ? $("#txt_num_ent_mipress").val() : "-1";//-1 el objeto está visible
        } else {
            num_ent_mipress = "";//-2 El objeto no está visible
        }
		
		var cmb_lista_merc = $("#hdd_categoria_mercadeo").val() != '' ? $("#hdd_categoria_mercadeo").val() : "NULL";
		var cmb_subcategoria_merc = $("#hdd_subcategoria_mercadeo").val() != '' ? $("#hdd_subcategoria_mercadeo").val() : "NULL";
		var txt_remitido_merc = $("#hdd_remitido_mercadeo").val() != '' ? $("#hdd_remitido_mercadeo").val() : "NULL";
		var txt_otro_merc = $("#hdd_otro_mercadeo").val() != '' ? $("#hdd_otro_mercadeo").val() : "NULL";
		var txt_referido_merc = $("#hdd_referido_mercadeo").val() != '' ? $("#hdd_referido_mercadeo").val() : "NULL";
		...
		var foto_paciente = $("#hdd_foto_paciente").val() != '' ? $("#hdd_foto_paciente").val() : "";
		...
		var params = "opcion=3&txtnombre1=" + str_encode($("#txt_nombre").val()) +
					 ...
					 //Datos del fromulario de mercadeo en la admision 
					 "&cmb_lista_merc="  + cmb_lista_merc +
					 "&cmb_subcategoria_merc="  + cmb_subcategoria_merc +
					 "&txt_referido_merc="  + txt_referido_merc +
					 "&txt_remitido_merc="  + txt_remitido_merc +
...
function verificar_guardar_admision() {
    ...
    if (resultado_aux > 0) {
        ...
        $("#contenedor_exito").html("Admisión registrada con éxito");
        ...
        $("#contenedor_error").css("display", "block");
        $("#contenedor_error").html("Error - No se encontraron profesionales disponibles para continuar con la atención");
        $("#btnNuevo").removeAttr("disabled");
        window.scrollTo(0, 0);
    } else if (resultado_aux == -11) {
        $("#contenedor_error").css("display", "block");
        $("#contenedor_error").html("Error - El número de documento solamente debe contener caracteres numéricos");
        $("#btnNuevo").removeAttr("disabled");
        window.scrollTo(0, 0);
    } else if (resultado_aux == -12) {
        $("#contenedor_error").css("display", "block");
        $("#contenedor_error").html("Error - El número de documento tiene una longitud incorrecta");
        $("#btnNuevo").removeAttr("disabled");
        window.scrollTo(0, 0);
        ...
        $("#contenedor_error").css("display", "block");
        $("#contenedor_error").html("Error interno al tratar de registrar la admisión (Código de error: " + resultado_aux + ")");
        $("#btnNuevo").removeAttr("disabled");
        window.scrollTo(0, 0);
    }
}
...
function verifica() { //Verifica si el usuario Existe
    ...
    if (pacienteExiste[0] == 1) {
        $("#contenedor_exito").css("display", "block");
        $("#contenedor_exito").html('Existe un paciente con número de documento <span style="font-weight: bold;">' + txt_id + '</span> y nombre <span style="font-weight: bold;">' + txt_nombre + "</span>.<br />Por favor verifique que se trata de la misma persona y actualice los datos estadísticos del paciente.");
        $("#btnBuscarusuario").css("display", "none");
        ...
        $("#contenedor_confirmacion").html(
			'<h5>Se encontró un paciente con ' + pacienteExiste[3] + ' número ' + pacienteExiste[2] + ' y nombre ' + pacienteExiste[1] + '</h5>' +
			'<h5> ¿Desea cargar los datos de este paciente? </h5>' +
			'<div id="confirmacion">' +
    ...
    if ($("#hdd_paciente_existe").val() == 3) {//Verifica si hdd_paciente_existe esta con valor de 3 = el paciente no existe
        $("#contenedor_advertencia").css("display", "block");
        $("#contenedor_advertencia").html("No se halló historia clínica, pregunte al paciente si es la primera vez que asiste al consultorio.");
    }
...
function postVerificaDatos() {//Funcion que verifica si hay datos en la funcion ajax de la funcion verifica() js
    ...
    if ($("#hdd_datos_paciente").val() != "") {
        ...
        var codigohtml =
			'<div class="encabezado">' +
				'<h3>Existe un registro con la información</h3>' +
			'</div>' +
			'<table style="width:100%;">' +
				...
				'</tr>' +
				'<tr>' +
					'<td style="text-align:right;">Número de documento:</td>' +
...
function opcionesFormularioflotante(n1) {
    if (n1 == 1) {
        ...
        if (datosPacienteCedula[0] != "0") {
			...
			$("#hdd_foto_paciente").val(resultado[27]);
            $("#cmb_numerohijos").val(resultado[28]);
            $("#cmb_numerohijas").val(resultado[37]);
            $("#cmb_numerohermanos").val(resultado[29]);
            $("#cmb_numerohermanas").val(resultado[38]);
            $("#cmb_estado_civil").val(trim(resultado[30]));
            $("#cmb_id_pais_nac").val(resultado[31]);
            seleccionar_pais(resultado[31], "_nac");
            $("#cmb_departamento_nac").val(resultado[32]);
            seleccionar_departamento(resultado[32], resultado[33], "_nac");
            $("#cmb_municipio_nac").val(resultado[33]);
            $("#txt_nombre_dep_nac").val(resultado[34]);
            $("#txt_nombre_mun_nac").val(resultado[35]);
			
			$("#hdd_categoria_mercadeo").val(resultado[39]);
			$("#hdd_subcategoria_mercadeo").val(resultado[40]);
			$("#hdd_remitido_mercadeo").val(resultado[41]);
			$("#hdd_referido_mercadeo").val(resultado[42]);
			...
			$("#hdd_foto_paciente").val(resultado[27]);
            $("#cmb_numerohijos").val(trim(resultado[28]));
            $("#cmb_numerohijas").val(trim(resultado[37]));
            $("#cmb_numerohermanos").val(trim(resultado[29]));
            $("#cmb_numerohermanas").val(trim(resultado[28]));
            $("#cmb_estado_civil").val(trim(resultado[30]));
            $("#cmb_id_pais_nac").val(resultado[31]);
            seleccionar_pais(resultado[31], "_nac");
            $("#cmb_departamento_nac").val(resultado[32]);
        ...
        $("#hdd_id_paciente").val(resultado[36]);
...
function seleccionar_convenio(id_plan, modificar_valores) {
	...
	if (ind_num_carnet == "1") {
		...
		$("#td_num_poliza_l").css("display", "table-cell");
		$("#td_num_poliza").css("display", "table-cell");
		
		$("#td_num_mipress_l").css("display", "table-cell");
		$("#td_num_mipress").css("display", "table-cell");
		
		$("#td_num_ent_mipress_l").css("display", "table-cell");
		$("#td_num_ent_mipress").css("display", "table-cell");
		...
		$("#td_num_poliza_l").css("display", "none");
		$("#td_num_poliza").css("display", "none");
		$("#txt_num_poliza").val("");
		
		$("#td_num_mipress_l").css("display", "none");
		$("#td_num_mipress").css("display", "none");
		$("#txt_num_mipress").val("");
		
		$("#td_num_ent_mipress_l").css("display", "none");
		$("#td_num_ent_mipress").css("display", "none");
		$("#txt_num_ent_mipress").val("");
...
function cargar_precios(modificar, ind_inicial) {
	...
	var tipo_coti_paciente = $("#cmb_tipo_coti_paciente").val();
    var rango_paciente = $("#cmb_rango_paciente").val();
	var ind_np_atencion = $("#chk_np_atencion").is(":checked") ? 1 : 0;
    ...
    if (tipoCita.length > 0) {
        if (id_plan.length > 0) {
            ...
            var params = "opcion=8&tipoCita=" + tipoCita +
						 ...
						 "&tipo_coti_paciente=" + tipo_coti_paciente +
						 "&rango_paciente=" + rango_paciente +
						 "&ind_np_atencion=" + ind_np_atencion;
...
function finalizar_actualizar_precio_det(indice) {
	...
	var tipo_precio = $("#hdd_tipo_precio_b_" + indice).val();
	...
	var bol_np_atencion = $("#chk_np_atencion").is(":checked");
	var tipo_precio_b = $("#hdd_tipo_precio_b_" + indice).val();

	$("#hdd_id_precio_" + indice).val(id_precio);
	$("#txt_valor_pago_" + indice).val(bol_np_atencion ? 0 : valor);
	$("#txt_valor_cuota_" + indice).val(bol_np_atencion ? 0 : valor_cuota);
...
function marcar_np_atencion(ind_modificar_script) {
	if ($("#chk_np_atencion").is(":checked")) {
		var cant_precios = parseInt($("#hdd_ult_index_precio").val(), 10);
		for (var i = 1; i <= cant_precios; i++) {
			$("#txt_valor_pago_" + i).val(0);
			$("#txt_valor_cuota_" + i).val(0);
		}
		recalcular_totales_liquidacion();
	} else {
		cargar_precios(ind_modificar_script, 0);
	}
}
...
function abrirCamara(){
	var numero_documento = $("#txt_id").val();
	if(numero_documento != "" && numero_documento != undefined){
		var idDispositivo = $("#listaDeDispositivos").val();
		turnOnCamera(idDispositivo); 
	}else{
		swal({
			title: "Error al abrir la c&aacute;mara",
			type: "error",
			text: "Debe llenar los datos del paciente",
			showConfirmButton: false,
			showCloseButton: true,
		});	
	}
...
function tomarFoto(){
	...
	if(numero_documento != "" && numero_documento != undefined){
		...
		var id_paciente = $("#hdd_id_paciente").val();
		var params = "opcion=24&id_paciente=" + id_paciente +
					 "&numero_documento="+numero_documento +
					 "&data=" + data;
   		llamarAjax("admision_ajax.php", params, "d_interno", "terminar_guardar_foto();");
	}  	
}

function terminar_guardar_foto(){
	
	$("#hdd_foto_paciente").val($("#foto_paciente_saved").val());
	if($("#foto_paciente_saved").val() != ""){
		 $('#resultado').fadeOut(1000, function () {
			 $('#camara').fadeIn(1000, function () {
					 swal({
							title: "Se ha tomado la foto correctamente",
							type: "success",
							text: "",
							showConfirmButton: false,
							showCloseButton: true,
		...
		swal({
				title: "Ha habido un error al intentar capturar la foto",
				type: "error",
				text: "",
				showConfirmButton: false,
				showCloseButton: true,
...
function turnOnCamera(idDispositivo){
	 canvas = document.getElementById("resultado");
	 cxt = canvas.getContext("2d");
	 video = document.getElementById("camara");
	...
	if(navigator.getUserMedia){	
		...
		$("#tr_camara").css("display", "block"); 
		$("#btn-abrir-camara").css("display", "none"); 
		$("#tr_tomar_foto").css("display", "block"); 
		$("#tr_dispositivos").css("display", "block"); 
		
		if($("#hdd_cargar_select").val() == ""){
				 $("#hdd_cargar_select").val("1"); 
				cargarDispositivos();
		}
		...
		alert("No existen los permisos para acceder a la cámara");
	}
}
...
function turnOffCamera(){
	...
	$("#btn-abrir-camara").css("display", "block"); 
	$("#tr_tomar_foto").css("display", "none");  
	$("#tr_dispositivos").css("display", "none"); 
	$("#tr_camara").css("display", "none"); 
}
...
function cargarDispositivos(){
	...
	//$('#listaDeDispositivos').empty().append("<option value='1'>Seleccione--</option>");	 	
	dispositivos.forEach(function (dispositivo) {
		...
		if (dispositivosDeVideo.length > 0) {
			...
			for(var i in dispositivosDeVideo){
					
			var option = document.createElement('option');
				option.value = dispositivosDeVideo[i].deviceId;
				option.text = dispositivosDeVideo[i].label;
