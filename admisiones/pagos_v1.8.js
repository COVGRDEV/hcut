...
function validar_registrar_pago(id, tipo_accion) {
		...
		for (var i = 0; i < cont_medios_pago; i++) {
			...
			if (id_tipo_pago_aux != "") {
				...
				if ($("#tipoPago" + id + "_" + i).val() == "" ||
						...
						&& $("#cmb_tipo_pago" + id + "_" + i).val() != "100")) {
						$("#tipoPago" + id + "_" + i).addClass("borde_error")
						ind_error = true;
				}
				if (($("#cmb_tipo_pago" + id + "_" + i).val() == "99" || $("#cmb_tipo_pago" + id + "_" + i).val() == "100") && $("#tipoPago" + id + "_" + i).val() != "0") {
	...
	if (tipo_accion != 3 && tipo_accion != 4 && tipo_accion != 5) {
		...
		if (!validar_email($("#txt_email").val())) {
				$("#txt_email").addClass("bordeAdmision");
				alert("Verifique que el correo tenga un formato válido.");
				ind_error = true;
			} else {
...
function RegistrarPago(idContador, tipo_accion) {
	if (!banderaClick){ //Validar de que solo se pueda hacer un click
		...
		var params = "opcion=2&id_pago=" + $("#hdd_id_pago").val() +
					 ...
					 "&num_mipress=" + str_encode($("#txt_num_mipress").length ? $("#txt_num_mipress").val() : "") +
					 "&num_ent_mipress=" + str_encode($("#txt_num_ent_mipress").length ? $("#txt_num_ent_mipress").val() : "") +
					 "&num_poliza=" + str_encode($("#txt_num_poliza").length ? $("#txt_num_poliza").val() : "") +
				...
				if ($("#hdd_resul_tercero_siesa").length && $("#hdd_resul_tercero_siesa").val() != "1") {
					...
					if ($("#hdd_resul_tercero_entidad_siesa").length && $("#hdd_resul_tercero_entidad_siesa").val() != "1") {
						...
						if ($("#hdd_resul_factura_siesa").length && $("#hdd_resul_factura_siesa").val() != "1") {
							...
							if($("#hdd_resul_entidad_factura_siesa").length && $("#hdd_resul_entidad_factura_siesa").val() != "1"){							
								$("#contenedor_error").html("Error en el registro externo en la entidad de la factura <br />" + $("#hdd_mensaje_entidad_factura_siesa").val());
							}else {
								if ($("#hdd_resul_pedido_siesa").length && $("#hdd_resul_pedido_siesa").val() != "1") {
									...
									if($("#hdd_resul_pedido_entidad_siesa").length && $("#hdd_resul_pedido_entidad_siesa").val() != "1"){
										$("#contenedor_error").html("Error en el registro externo de la entidad del pedido <br />" + $("#hdd_mensaje_pedido_entidad_siesa").val());
...
function tipoPago(idcontador, indice, id_pago, estado_pago) {
	if ($("#cmb_tipo_pago" + idcontador + "_" + indice).val() != "") { //Si el valor es seleccionado
		...
		if (!ind_repetido_aux && ind_tipo_rel) {
			if (id_tipo == "99" || id_tipo == "100") { //No pago
...
function continuar_borrar_pago() {
	...
	let num_nota_credito = $("#txt_nota_credito").val() == undefined ? "" : $("#txt_nota_credito").val().trim();
	var params = "opcion=11&id_pago=" + $("#hdd_id_pago").val() +
				 "&observaciones_pago=" + str_encode($("#txt_observaciones_pago").val()) +
				  "&num_nota_credito=" + str_encode(num_nota_credito) +
...
function consultar_pago_siesa(id_pago, id_lugar_cita, numero_documento) {
	...
	llamarAjax("pagos_ajax.php", params, "d_tipo_pago_siesa", "continuarConsultarPagoSiesa()");
}

function continuarConsultarPagoSiesa() {
	let num_pedido = $("#hdd_num_pedido_siesa").val().trim();
	let num_factura = $("#hdd_num_factura_siesa").val().trim();
	let num_nota = $("#hdd_num_nota_siesa").val().trim();
	let id_pago = $("#hdd_id_pago").val().trim();
	
	var params = "opcion=16&id_pago=" + id_pago +
...
function finalizar_consultar_pago_siesa() {
	...
	if(parseInt($("#hdd_result_actualizar_pago").val()) > 0){
		
		seleccionar_paciente($("#hdd_id_pago").val().trim());
	}
