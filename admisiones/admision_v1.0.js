    ...
    $("#frmAdmision").validate({
        ...
        submitHandler: function (form) {
            ...
            /*
             ...
             cmb_departamento_nac = "NULL";//-2 El objeto no esta visible
             }
             ...
             cmb_municipio_nac = "NULL";//-2 El objeto no esta visible
             }
             ...
             txt_nombre_dep_nac = "NULL";//-2 El objeto no esta visible
             }
             ...
             txt_nombre_mun_nac = "NULL";//-2 El objeto no esta visible
             }*/
            ...
            /*
             ...
             txt_num_carnet = $("#txt_num_carnet").val().length > 0 ? $("#txt_num_carnet").val() : "NULL";
             }*/
            ...
            if ($("#cmd_profesionalAtiende").is(":visible")) {
                ...
                cmd_profesionalAtiende = "NULL";//-2 El objeto no esta visible
            }
            ...
            /*
             cmb_departamento_nac === "-1" || cmb_municipio_nac === "-1" || txt_nombre_dep_nac === "-1" || txt_nombre_mun_nac === "-1"
             || || txt_nombre_dep === "-1" || txt_nombre_mun === "-1"
             || cmb_desplazado === "-1" || cmb_etnia === "-1" || txt_nombre_cirugia === "-1"
             || txt_fecha_cirugia === "-1" || cmb_ojo === "-1" || cmb_num_cirugia === "-1" || txt_nombre_med_orden === "-1"
             || cmb_cant_examenes === "-1" || ind_examenes_compl == "-1" || txt_num_carnet === "-1"
             
             */

            if (cmb_departamento === "-1" || cmb_municipio === "-1" || cmd_profesionalAtiende === "-1" || ind_profesionales_sel == "-1") {
                ...
                /*
                 if (cmb_cant_examenes != "NULL" && cmb_cant_examenes != "") {
                 for (var i = 0; i < parseInt(cmb_cant_examenes, 10); i++) {
                 if ($("#cmb_examen_" + i).val() == "") {
...
function continuar_guardar_admision() {
    ...
    //var medioPago = $("#cmb_medio_pago").val();
    var medioPago = 75;//Efectivo

    var params = "opcion=11&id_medio_pago=" + medioPago;
    var arrProductos = "";
    var arrTamano = filas;
    for (i = 1; i <= filas; i++) {
        if (isObject(document.getElementById("tr_precios_" + i))) {
            ...
            var idListaPrecios = $("#hdd_id_precio_" + i).val().length > 0 ? $("#hdd_id_precio_" + i).val() : 0;
            var id_convenio_pago = $("#cmb_convenio_pago_" + i).val().length > 0 ? $("#cmb_convenio_pago_" + i).val() : 0;
            var id_plan_pago = $("#cmb_plan_pago_" + i).val().length > 0 ? $("#cmb_plan_pago_" + i).val() : 0;
            var num_autorizacion = $("#txt_num_autorizacion_" + i).val().length > 0 ? str_encode($("#txt_num_autorizacion_" + i).val()) : 0;
            var cantidad = $("#txt_cant_precio_" + i).val().length > 0 ? $("#txt_cant_precio_" + i).val() : 0;
            var tipoPrecio = $("#hdd_tipo_precio_" + i).val().length > 0 ? trim($("#hdd_tipo_precio_" + i).val()) : 0;
            var cod_servicio = $("#hdd_cod_servicio_" + i).val().length > 0 ? $("#hdd_cod_servicio_" + i).val() : 0;
            var tipoBilateral = $("#hdd_tipo_bilateral_" + i).val().length > 0 ? trim($("#hdd_tipo_bilateral_" + i).val()) : 0;
            var valor = $("#txt_valor_pago_" + i).val().length > 0 ? $("#txt_valor_pago_" + i).val() : 0;
            var valor_cuota = $("#txt_valor_cuota_" + i).val().length > 0 ? $("#txt_valor_cuota_" + i).val() : 0;
            var indTipoPago = $("#hdd_tipo_pago_" + i).val().length > 0 ? $("#hdd_tipo_pago_" + i).val() : 0;

            arrProductos += idListaPrecios + "*" + id_convenio_pago + "*" + id_plan_pago + "*" + num_autorizacion + "*" + cantidad + "*" + tipoPrecio + "*" + cod_servicio + "*" + tipoBilateral + "*" + valor + "*" + valor_cuota + "*" + indTipoPago + (filas > 1 ? (i >= filas ? "" : "|") : "");
            cont_aux++;
        }
    }
    params += "&cant_precios=" + cont_aux;
    //alert(arrProductos+" -- -- "+arrTamano);
    continuar_guardar_admision_2(arrProductos, arrTamano);
    //llamarAjax("admision_ajax.php", params, "d_guardar_precios", "continuar_guardar_admision_2();");
}

function continuar_guardar_admision_2(arrProductos, arrTamano) {//arrProductos, arrTamano
    ...
    if (tmp_pagos_detalle <= 0) {
        ...
        if ($("#cmb_desplazado").is(":visible")) {
            ...
            cmbdesplazado = "NULL";//-2 El objeto no esta visible
        }

        if ($("#cmb_etnia").is(":visible")) {
            cmbetnia = $("#cmb_etnia").val().length > 0 ? $("#cmb_etnia").val() : "-1";//-1 el objeto esta visible
        } else {
            cmbetnia = "NULL";//-2 El objeto no esta visible
        }

        if ($("#cmd_profesionalAtiende").is(":visible")) {
            cmbprofesionalAtiende = $("#cmd_profesionalAtiende").val().length > 0 ? $("#cmd_profesionalAtiende").val() : "-1";//-1 el objeto esta visible
        } else {
            cmbprofesionalAtiende = "NULL";//-2 El objeto no esta visible
        }

        if ($("#td_presion_arterial").is(":visible")) {
            txt_presionarterial = $("#txt_presionarterial").val().length > 0 ? $("#txt_presionarterial").val() : "NULL";
        } else {
            txt_presionarterial = "NULL";
        }

        if ($("#td_pulso").is(":visible")) {
            txt_pulso = $("#txt_pulso").val().length > 0 ? $("#txt_pulso").val() : "NULL";
        } else {
            txt_pulso = "NULL";
        }
        ...
        var params = "opcion=3&txtnombre1=" + str_encode($("#txt_nombre").val()) +
                ...
                "&cmbetnia=" + cmbetnia +
                "&cmbzona=" + 'NULL'/*$("#cmb_zona").val()*/ +
                "&hddidconsultaaux=" + hddidconsultaaux[1] +
                "&txttelefono2=" + 'NULL'/*str_encode($("#txt_telefono2").val())*/ +
                "&txtemail=" + 'NULL'/*str_encode($("#txt_email").val())*/ +
                "&id_cita=" + hddidconsultaaux[0] +
                "&id_tipo_cita=" + $("#cmb_tipo_cita").val() +
                "&hddpacienteexisteaux=" + hddpacienteexisteaux[0] +
                "&cmbtiposangre=" + 'NULL'/*$("#cmb_tipo_sangre").val()*/ +
                ...
                "&cmb_cant_examenes=" + 'NULL' /*cmb_cant_examenes*/ +
                "&cmbnumerohijos=" + 'NULL'/*$("#cmb_numerohijos").val()*/ +
                "&cmbnumerohijas=" + 'NULL' /*$("#cmb_numerohijas").val()*/ +
                "&cmbnumerohermanos=" + 'NULL' /*$("#cmb_numerohermanos").val()*/ +
                "&cmbnumerohermanas=" + 'NULL' /*$("#cmb_numerohermanas").val()*/ +
                "&txtacompanante=" + 'NULL' /*str_encode($("#txt_acompanante").val())*/ +
                "&txtplan=" + $("#cmb_plan").val() +
                "&factorrh=" + 'NULL' /*$("#cmb_factor_rh").val()*/ +
                "&cmb_lugar_cita=" + $("#cmb_lugar_cita").val() +
                "&cmbconvenio=" + $("#cmb_convenio").val() +
                "&txtprofesion=" + 'NULL'/*str_encode($("#txt_profesion").val())*/ +
                ...
                "&cmb_estado_civil=" + 'NULL' /*$("#cmb_estado_civil").val()*/ +
                "&txt_num_carnet=" + 'NULL' /*txt_num_carnet*/ +
                "&exento=" + exento +
        ...
        //llamarAjax("admision_ajax.php", params, "sqle", "");
    }
}
...
function verificar_guardar_admision() {
    ...
    if (resultado_aux > 0) {
        ...
        $("#btnNuevo").removeAttr("disabled");
        window.scrollTo(0, 0);
    }else if (resultado_aux == -15) {
        $("#contenedor_error").css("display", "block");
        $("#contenedor_error").html("Error - Eror en el procedimiento de almacenado Pagos");
        $("#btnNuevo").removeAttr("disabled");
        window.scrollTo(0, 0);
    }
    else if (resultado_aux == -16) {
        $("#contenedor_error").css("display", "block");
        $("#contenedor_error").html("Error - El tipo de cita seleccionado no tiene parametrizado el detalle. (Código de error: " + resultado_aux + ")");
        $("#btnNuevo").removeAttr("disabled");
        window.scrollTo(0, 0);
    }else {
        $("#contenedor_error").css("display", "block");
...
function verificarCuotaModeradora() {
    var indCuotaModeradora = $("#hdd_ind_cuota_moderadora").val();
    if (indCuotaModeradora == 0) {
        $(".cuotaM").css("display", "none");
        $("#cmb_rango").val("");
        $("#cmb_exento").val("");
    } else {
        $(".cuotaM").css("display", "table-cell");
    }
}

function seleccionar_convenio(id_plan, modificar_valores) {
    var id_convenio = $("#cmb_convenio").val();
    var params = "opcion=5&codigoConvenio=" + id_convenio + "&id_plan=" + id_plan + "&modificar_valores=" + modificar_valores;

    llamarAjax("admision_ajax.php", params, "d_plan", "borrarPrecios();verificarCuotaModeradora();");
...
function cargar_precios(modificar, ind_inicial) {
    ...
    var statusConvenio = $("#hdd_status_convenio").val();
    var exento = $("#hdd_exento").val();
    var rango = $("#hdd_rango").val();

    if (tipoCita.length > 0) {
        if (id_plan.length > 0) {
            var tipoCita = $("#cmb_tipo_cita").val();
            var params = "opcion=8&tipoCita=" + tipoCita +
                    //var params = "opcion=20&tipoCita=" + tipoCita +
    ...
    for (i = 1; i < filas; i++) {
        if ($("#hdd_sel_proc_b_" + i).is(":checked")) {
            ...
            $("#tablaPrecios").append(
                    ...
                    '<input type="text" name="txt_cant_precio_' + cont_aux + '" id="txt_cant_precio_' + cont_aux + '" value="1" class="texto_centrado no-margin" maxlength="3" onkeypress="return solo_numeros(event, false);" />\n' +
                    '</td>\n' +
                    '<td class="td_reducido">\n' +
                    '<input type="hidden" name="hdd_tipo_pago_' + cont_aux + '" id="hdd_tipo_pago_' + cont_aux + '" value="' + $("#hdd_ind_tipo_pago_b_" + i).val() + '" />\n' +
                    '<input type="text" name="txt_valor_pago_' + cont_aux + '" id="txt_valor_pago_' + cont_aux + '" value="' + $("#hdd_precio_b_" + i).val() + '" class="texto_centrado no-margin" maxlength="9" onkeypress="return solo_numeros(event, false);" />\n' +
                    '</td>\n' +
                    '<td>\n' +
                    '<input type="text" name="txt_valor_cuota_' + cont_aux + '" id="txt_valor_cuota_' + cont_aux + '" value="' + $("#hdd_precio_cuota_b_" + i).val() + '" class="texto_centrado no-margin" maxlength="9" onkeypress="return solo_numeros(event, false);" ' + disabled_aux + ' />\n' +
                    '</td>\n' +
                    '<td style="width: 40px;">\n' +
...
function cargar_usuarios_tipo_cita_admision(id_admision, tipo_cita, id_profesional, estado_consulta) {
    ...
    params += "&id_usuario_prof=" + id_profesional;
    params += "&id_admision=" + id_admision;
    params += "&estado_consulta=" + estado_consulta;
