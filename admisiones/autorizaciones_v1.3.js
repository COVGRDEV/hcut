...
function generar_pdf_autorizacion() {
	var ruta_sala = $("#hdd_ruta_autorizacion_sala_pdf").val();
	var ruta_anestesia = $("#hdd_ruta_autorizacion_anestesia_pdf").val();
	var ruta_especialista = $("#hdd_ruta_autorizacion_especialista_pdf").val();

		
	if(ruta_sala != undefined && ruta_sala != ""){window.open("../funciones/abrir_pdf.php?ruta=" + ruta_sala +        "&nombre_arch=ordenMeicamentosSala.pdf", "_blank");}
	
	if(ruta_anestesia != undefined && ruta_anestesia != ""){window.open("../funciones/abrir_pdf.php?ruta=" +         ruta_anestesia + "&nombre_arch=ordenMeicamentosAnestesiologo.pdf", "_blank");}
	
	if(ruta_especialista != undefined && ruta_especialista != ""){window.open("../funciones/abrir_pdf.php?ruta=" +         ruta_especialista + "&nombre_arch=ordenMeicamentosEspecialista.pdf", "_blank");}
...
function autorizar_procedimiento_adicional() {
	...
	var ojo = $("#cmb_ojo").val();
	var idProveedorSala = $("#codProvModalSala").val();
	var idProveedorAnestesiologo = $("#codProvModalAnestesia").val();
	var idProveedorEspecialista = $("#codProvModalEspecialista").val();
		
	var banderaProveedor = false;
	
	idProveedorSala || idProveedorAnestesiologo || idProveedorEspecialista != "" 
	? banderaProveedor = true : banderaProveedor; 

	if (banderaProveedor && ciex.length > 0 && ojo.length > 0 && banderaProveedor) {
		swal({
				...
				swal({//Ventana temporal que muestra el progreso del ajax...
					...
					onOpen: () => {
						...
						var params = "opcion=15&tipoAuto=" + tipoAuto +
									 ...
									 "&idProveedorSala=" + idProveedorSala +
									 "&idProveedorAnestesiologo=" + idProveedorAnestesiologo +
									 "&idProveedorEspecialista=" + idProveedorEspecialista +
...
function btnVentanaBuscarProveedor(opcion) {
	swal({//Ventana temporal que muestra el progreso del ajax...
		...
		cancelButtonText: "Cancelar",
		onOpen: () => {
			var params = "opcion=14&case=" + opcion;
...
function buscarProveedor(opcion) {
...
function agregarProveedorAutorizaciones(id_proveedor, numero_documento, nombre_proveedor, opcion) {
	swal.close();
	switch(opcion) {
	  case 1: //SALA
		$("#codProvModalSala").val(id_proveedor);
		$("#nomProvModalSala").html(nombre_proveedor);
	  break;
	  case 2://ANESTESIA
		$("#codProvModalAnestesia").val(id_proveedor);
		$("#nomProvModalAnestesia").html(nombre_proveedor);
	  break;
	  case 3://ESPECIALISTA
		$("#codProvModalEspecialista").val(id_proveedor);
		$("#nomProvModalEspecialista").html(nombre_proveedor);
	  break;
	  case 4://MATERIALES
		$("#codProvModalMateriales").val(id_proveedor);
		$("#nomProvModalMateriales").html(nombre_proveedor);
	  break;
	}
		...
		swal({
			...
			if (resultado.value) {
				...
				var params = "opcion=18&id_paciente=" + $("#hdd_paciente").val() +
							 ...
							 "&id_paquete=" + $("#id_paquete_procedimiento").val();
...
function enviar_formulario_registrar_pago(id_admision, id_pago) {
	...
	$("#frm_credencial").append('<input type="hidden" id="hdd_ind_pago_auto" name="hdd_ind_pago_auto" value="1"  />');
...
function mostrarProveedorModal(opcion){

	switch(opcion) {
		
		case 1:
			$('#checkAutoDerechosSala').prop('checked') ? $('#tr_codProvModal_sala').css('display','') :  $('#tr_codProvModal_sala').css('display','none');
			
		break;
		case 2:
			$('#checkAutoAnestesiologo').prop('checked') ? $('#tr_codProvModal_anestesia').css('display','') :  $('#tr_codProvModal_anestesia').css('display','none');
		case 3:
			$('#checkAutoEspecialista').prop('checked') ? $('#tr_codProvModal_especialista').css('display','') :  $('#tr_codProvModal_especialista').css('display','none');
		break;
...
function renovar_autorizacion(){
	var cantidad_meses = $("#select_renovar_auto").val();
	var id_ordern_medica = $("#hddIdOrdenMedica").val(); 
	var params = "opcion=23"+"&cantidad_meses="+cantidad_meses
					+"&id_ordern_medica="+id_ordern_medica;
          
    llamarAjax("../admisiones/autorizaciones_ajax.php", params, "d_renovar", "confirmarRenovacion();");
	
	
}
function confirmarRenovacion(){
	var resultado = $("#hdd_result_renovacion").val();
	
	if(resultado>0){
		swal({
			title: "Se ha renovado la autorización con éxito",
			type: "success",
			showCloseButton: true,
			showCancelButton: false,
			confirmButtonText: "Aceptar",
		...
		swal({
			title: resultado + " Ha ocurrido un error",
			type: "error",
			showCloseButton: true,
			showCancelButton: false,
			confirmButtonText: "Aceptar",
...
function actualizar_autorizacion(){
	var idAuto = $("#hdd_idautorizacion").val();
	var idProvSala = $("#codProvModalSala").val();
	var idProvAnes = $("#codProvModalAnestesia").val();
	var idProvEsp = $("#codProvModalEspecialista").val();
	var idProvMat = $("#codProvModalMateriales").val();
	var idOjo = $("#cmb_ojo").val();
	
	idProvSala = idProvSala == undefined ? "" : idProvSala;
	idProvAnes = idProvAnes == undefined ? "" : idProvAnes;
	idProvEsp = idProvEsp == undefined ? "" : idProvEsp;
	idProvMat = idProvMat == undefined ? "" : idProvMat;
	idOjo = idOjo == undefined ? "" : idOjo;
	
	var params = "opcion=24&id_prov_sala="+idProvSala +
					"&id_prov_anes="+idProvAnes  +
					"&id_prov_esp="+idProvEsp  +
					"&id_prov_mat="+idProvMat +
					"&idOjo="+idOjo +
					"&id_auto=" +idAuto;

    llamarAjax("../admisiones/autorizaciones_ajax.php", params, "d_impresion_hc", "continuar_actualizar_autorizacion();");
}

function continuar_actualizar_autorizacion(){
	if($("#hdd_result_actualizacion").val()>0){
		swal({
			title: "Se han actualizado los proveedores de manera exitosa.",
			type: "success",
			showCloseButton: true,
			showCancelButton: false,
			confirmButtonText: "Aceptar",
