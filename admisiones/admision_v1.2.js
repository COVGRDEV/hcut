...
function mostrarFormularioMercadeo(){
	...
	var params = "opcion=21&flotante=" + 1  + 
				  ...
				  "&remitido="      +  remitido         +
				  "&otro="          +  otro;
	if(categoria=="95" || categoria=="89"){
		llamarAjax("admision_ajax.php", params, "d_interno", "seleccionar_mercadeo()");
	}else{
...
function continuar_guardar_admision_2() {
	...
	//Datos del formulario de mercadeo
	 ...
	 if(id_lugar_session === "491" || id_lugar_session === "466" || id_lugar_session === "511" || id_lugar_session === "465" || id_lugar_session === "464" ){
			...
			//var otro = $("#hdd_otro_" + id_paciente).val();
	 }else{
...
function verificar_guardar_admision() {
    ...
    var resultado_aux = ($("#hdd_resultado_crear").val() != "" ? parseInt($("#hdd_resultado_crear").val(), 10) : -100);
	 var resultado_aux_merc = ($("#hdd_resultado_crear_form_mercadeo").val() != "" ? parseInt($("#hdd_resultado_crear_form_mercadeo").val(), 10) : -400);
	alert(resultado_aux);
	
    if (resultado_aux > 0 && resultado_aux_merc>0) {
        ...
        $("#btnNuevo").removeAttr("disabled");
        window.scrollTo(0, 0);
    } else if (resultado_aux_merc == -400) {
        $("#contenedor_error").css("display", "block");
        $("#contenedor_error").html("Error - Falló al intentar guardar los datos de mercadeo");
        $("#btnNuevo").removeAttr("disabled");
        window.scrollTo(0, 0);
    } else {
        $("#contenedor_error").css("display", "block");
