...
function realizar_consulta_prog_cx() {
    if ($("#txt_fecha_ini").val() == "" || $("#txt_fecha_fin").val() == "") {
        alert("Debe seleccionar la fecha inicial y la fecha final");
    } else {
		params = "opcion=3&fecha_ini=" + $("#txt_fecha_ini").val() +
				 "&fecha_fin=" + $("#txt_fecha_fin").val() +
				 "&tipo_fecha=" + $("input[name=rad_tipo_fecha]:checked", "#frm_tipos_fecha").val() +
				 "&cod_concepto=" + $("#hdd_cod_concepto").val() +
				 "&tipo_concepto=" + $("#hdd_tipo_concepto").val() +
				 "&id_usuario_prof=" + $("#cmb_lista_usuarios").val();
		
		var cant_estados_prog = parseInt($("#hdd_cant_estados_prog").val(), 10);
		for (var i = 0; i < cant_estados_prog; i++) {
			params += "&estado_prog_" + i + "=" + ($("#chk_estado_prog_" + i).is(":checked") ? 1 : 0);
		}
		params += "&cant_estados_prog=" + cant_estados_prog;
		
		llamarAjax("reporte_programacion_cx_ajax.php", params, "d_buscar_prog_cx", "");
    }
}
...
function generar_reporte_excel() {
    if ($("#txt_fecha_ini").val() == "" || $("#txt_fecha_fin").val() == "") {
		...
		$("#hdd_cod_concepto_r").val($("#hdd_cod_concepto").val());
		$("#hdd_tipo_concepto_r").val($("#hdd_tipo_concepto").val());
        $("#hdd_id_usuario_prof").val($("#cmb_lista_usuarios").val());
