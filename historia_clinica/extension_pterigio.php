...
<table border="0" cellpadding="3" cellspacing="0" align="center" style="width:98%;">
	...
	<tr>
		<td valign="top">
			<div class="contenedor_uploader">
				<div id="uploader_pte_foto_od"></div>				
				<div class="div_botonera_adic">
					<input type="button" id="btn_imagenes_hc_od" value="Ver Archivo(s)" class="btnPrincipal peq btnUp" onclick="ver_archivos( <?php echo($pte_fotos_od["id_reg_archivos"].", 'HC', ".$id_hc_consulta.", 0, 65"); ?> )"/>
				</div>
			...
			<script>
				var uploader_pte_foto_od_default_params = {
					opcion: 91,
					idTipoAdjunto: <?php echo($pte_fotos_od_tipo_archivo); ?>,
					idHc: <?php echo($id_hc_consulta); ?>
				}
				
				var uploader_pte_foto_od = new qq.FineUploader({
					element: document.getElementById("uploader_pte_foto_od"),
					//debug: true,
					// Configuración de archivos iniciales en el componente:
					session: {
					...
					// Parámetros para cargue de archivos:
					request: {
						endpoint: 'extension_pterigio_ajax.php',
						method: 'POST',
						inputName: "pte_foto_od",
						params: uploader_pte_foto_od_default_params,
						paramsInBody: true
					},
					// Parámetros para borrado de archivos:
					deleteFile: {
						enabled: true,
						endpoint: 'extension_pterigio_ajax.php',
						method: 'POST',
					...
					messages: uploader_default_messages,
					validation: {
						allowedExtensions: ["jpeg", "jpg", "png", "gif", "pdf", "xls", "xlsx"],
		...
		<td valign="top">
			<div class="contenedor_uploader">
				<div id="uploader_pte_foto_oi"></div>				
				<div class="div_botonera_adic">
					<input type="button" id="btn_imagenes_hc_oi" value="Ver Archivo(s)" class="btnPrincipal peq btnUp" onclick="ver_archivos( <?php echo($pte_fotos_oi["id_reg_archivos"].", 'HC', ".$id_hc_consulta.", 0, 65"); ?> )"/>
				</div>
				<input type="hidden" id="pte_foto_tipo_arch_oi" name="pte_foto_tipo_arch_oi" value="<?php echo($pte_fotos_oi_tipo_archivo); ?>"> 
				<input type="hidden" id="pte_foto_id_lote_oi" name="pte_foto_id_lote_oi" value="<?php echo($pte_fotos_oi["id_reg_archivos"]); ?>"> 
			</div>		
			<script>
				var uploader_pte_foto_oi_default_params = {
					opcion: 91,
					idTipoAdjunto: <?php echo($pte_fotos_oi_tipo_archivo); ?>,
					idHc: <?php echo($id_hc_consulta); ?>
				}
				
				var uploader_pte_foto_oi = new qq.FineUploader({
					element: document.getElementById("uploader_pte_foto_oi"),
					//debug: true,
					session: {
						endpoint: 'extension_pterigio_ajax.php',
						params: {
					...
					request: {
						endpoint: 'extension_pterigio_ajax.php',
						method: 'POST',
						inputName: "pte_foto_oi",
						params: uploader_pte_foto_oi_default_params,
						paramsInBody: true
					},
					deleteFile: {
						enabled: true,
