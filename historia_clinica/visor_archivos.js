...
function cargar_archivos(modulo, id_modulo, id_activo, retardo) {
	var params = "opcion=1&modulo=" + modulo + "&id_modulo=" + id_modulo + "&id_activo=" + id_activo; 
	//console.log(params); 
	if (retardo > 0) {
		setTimeout(function() {llamarAjax("visor_archivos_ajax.php", params, "d_ver_imagenes_hc", "");}, retardo);
	} else {
		llamarAjax("visor_archivos_ajax.php", params, "d_ver_imagenes_hc", ""); 
	}
}

function mostrar_detalle_nodo(id_nodo) {
	var ind_titulo_imagen_hc = parseInt($("#hdd_titulo_imagen_hc_" + id_nodo).val(), 10);
	
	if (ind_titulo_imagen_hc == 0) {
		//Mostrar detalle
		$("#img_titulo_imagen_hc_down_" + id_nodo).css("display", "none");
		$("#img_titulo_imagen_hc_up_" + id_nodo).css("display", "inline-block");
		$("#hdd_titulo_imagen_hc_" + id_nodo).val(1);
		$("#d_lista_imagen_hc_" + id_nodo).show(200);
	} else {
		//Ocultar detalle
		$("#img_titulo_imagen_hc_down_" + id_nodo).css("display", "inline-block");
		$("#img_titulo_imagen_hc_up_" + id_nodo).css("display", "none");
		$("#hdd_titulo_imagen_hc_" + id_nodo).val(0);
		$("#d_lista_imagen_hc_" + id_nodo).hide(200);
	}
}

function mostrar_archivo(id_examen_hc_det) {
	var cant_archivos_imagen_hc = parseInt($("#hdd_cant_archivos_imagen_hc").val(), 10); //$("[onclick*='mostrar_archivo(']")
	...
	//console.log("mostré = "+id_examen_hc_det_aux);
}

function mostrar_archivo_indice(indice) {	
	mostrar_archivo($("#hdd_id_examen_hc_det_" + indice).val()); 
}
...
function manejar_zoom_archivo(direccion, id) {
