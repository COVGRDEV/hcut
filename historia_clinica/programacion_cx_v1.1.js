...
function finalizar_verificar_paciente(ind_resultado) {
	...
	if (ind_resultado == 1) {
		...
		seleccionar_departamento($("#hdd_cod_dep_nac_b").val(), $("#hdd_cod_mun_nac_b").val(), "nac");
		...
		seleccionar_departamento($("#hdd_cod_dep_b").val(), $("#hdd_cod_mun_b").val(), "res");
...
function seleccionar_departamento(cod_dep, cod_mun_dane, sufijo) {
	var funcion_mun = "";
	if (cod_mun_dane != "") {
		funcion_mun = '$("#cmb_cod_mun_' + sufijo + '").val("' + cod_mun_dane + '");';
	}
	var params = "opcion=5&cod_dep=" + cod_dep +
				 "&sufijo=" + sufijo;
	
	llamarAjax("programacion_cx_ajax.php", params, "d_municipio_" + sufijo, funcion_mun);
}
