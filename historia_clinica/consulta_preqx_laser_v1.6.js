...
function validar_preqx_laser() {
	...
	$("#cke_patologia_ocular_descripcion").removeClass("borde_error");
	$("#cirugia_ocular_valor").removeClass("borde_error");
	$("#cke_cirugia_ocular_descripcion").removeClass("borde_error");	
	$("#cke_diagnostico_preqx_laser").removeClass("borde_error");
	...
	if ($("#nomograma_equipo").val() == "") {
		$("#nomograma_equipo").addClass("borde_error");
		result = 1;
		panel_3 = 1;
	}
	if ($("#patologia_ocular_valor").val() == "") {
		...
		if ($("#patologia_ocular_valor").val() == "45") {
			if (CKEDITOR.instances.patologia_ocular_descripcion.getData() == "") {
				$("#cke_patologia_ocular_descripcion").addClass("borde_error");
				result = 1;
				panel_3 = 1;
			}	
		}
	...
	if ($("#cirugia_ocular_valor").val() == "") {
		...
		if ($("#cirugia_ocular_valor").val() == "45") {
			if (CKEDITOR.instances.cirugia_ocular_descripcion.getData() == "") {
				$("#cke_cirugia_ocular_descripcion").addClass("borde_error");
				result = 1;
				panel_3 = 1;
			}
		}
...
function crear_preqx_laser(tipo, ind_imprimir) {
	switch (tipo) {
		...
		case 4: //Finalizar consulta desde traslado
			...
			/*var resultado = validar_preqx_laser();
			if (resultado == 0) {
				editar_consulta_preqx_laser(tipo, ind_imprimir);
				return false;
			} else {
...
function imprimir_preqx_laser() {
	...
	llamarAjax("../historia_clinica/impresion_historia_clinica.php", params, "d_impresion_hc", "continuar_imprimir_preqx_laser();");
}

function continuar_imprimir_preqx_laser() {
	var ruta = $("#hdd_ruta_arch_hc_pdf").val();
	window.open("../funciones/abrir_pdf.php?ruta=" + ruta + "&nombre_arch=consulta_preqx_laser_optometria.pdf", "_blank");
}
 ...
 * 3=Guardar y NO cambiar el estado de la consulta CON VALIDACION DE LOS CAMPOS
 */
function editar_consulta_preqx_laser(tipo, ind_imprimir) {
	...
	var patologia_ocular_descripcion = CKEDITOR.instances.patologia_ocular_descripcion.getData();
	var cirugia_ocular_valor = $("#cirugia_ocular_valor").val();
	var cirugia_ocular_descripcion = CKEDITOR.instances.cirugia_ocular_descripcion.getData();
	var paquimetria_central_od = $("#paquimetria_central_od").val();
	var paquimetria_central_oi = $("#paquimetria_central_oi").val();
	var diagnostico_preqx_laser = CKEDITOR.instances.diagnostico_preqx_laser.getData();
	...
	llamarAjax("consulta_preqx_laser_ajax.php", params, "guardar_preqx_laser", "validar_exito(" + ind_imprimir + ");");
}
...
function enviar_a_estados() {
	crear_preqx_laser(2, 0);
	
	var params = "opcion=3&id_hc=" + $("#hdd_id_hc_consulta").val() +
				 "&id_admision=" + $("#hdd_id_admision").val();
	
	llamarAjax("consulta_preqx_laser_ajax.php", params, "d_interno", "mostrar_formulario_flotante(1);");
}
